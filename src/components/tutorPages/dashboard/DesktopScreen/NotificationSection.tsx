...
type NotificationSectionProps = {
  tutorNotifications: NotificationType[] | undefined;
};
...
const NotificationSection = ({
  ...
  const handleClearAll = () => {
    ...
    const UnReadNotifications = tutorNotifications
  ...
  useEffect(() => {
    ...
    setTotalUnreadNotifications(
      tutorNotifications?.filter((notification) => notification.read === false)
        .length || 0
    );
    setShowMore(new Array(tutorNotifications?.length).fill(false));
  }, [tutorNotifications]);
  ...
  const handleReadNotification = (index: number) => async () => {
    ...
    const tutorId = auth.currentUser ? auth.currentUser.uid : "";
    const notificationId = tutorNotifications ? tutorNotifications?.[index].uid : "";
    const response = await markTutorNotificationRead({ tutorId: tutorId, notificationId: notificationId });
    if (response.type === "error") {
      console.log(response.message);
    }
      ...
      <div className="">
        <div className="flex  Cgap-3 items-center font-semibold">
          ...
          {showNotification ? (
            ...
            <h1 className="text-empoweredFlag ml-5">Notifications</h1>
          )}
        </div>
        {!tutorNotifications ? (
          // <Spinner size="sm" className="mt-0 " />.
          <div className="h-14 w-full rounded-lg bg-gray-300 animate-pulse"></div>
        ) : (
          <div
            className={`bg-white shadow-xl max-h-[480px] overflow-y-auto custom-scrolbar p-5  relative bottom-2 rounded-xl mx-auto
